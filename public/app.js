...
document.addEventListener('click',async event=>{
  ...
  if(el.dataset.deleteResource&&confirm('确定删除这份资源？关联的链接和点击统计也会删除，无法撤销。'))await busy(el,async()=>{await api('/api/admin/resources/'+el.dataset.deleteResource,'DELETE');location.reload();});
});
let dirty=false;
...
resourceForm?.addEventListener('submit',event=>{event.preventDefault();busy($('button[type=submit]',resourceForm),async()=>{const data=Object.fromEntries(new FormData(resourceForm));const id=resourceForm.dataset.id;const result=await api('/api/admin/resources'+(id?'/'+id:''),id?'PUT':'POST',data);dirty=false;toast('资源已发布');$('#form-status').textContent='保存成功';if(!id)location.href='/admin/edit/'+result.id;});});
...
function parseCSV(text){
  ...
  if(header.length!==2||!header.includes('title')||!header.includes('url'))throw new Error('请使用模板列名，只需 title 和 url 两列');
...
$('#csv-file')?.addEventListener('change',async event=>{const file=event.target.files[0];$('#confirm-import').hidden=true;$('#import-preview').textContent='';importRows=[];if(!file)return;
  try{if(file.size>1024*1024)throw new Error('CSV 文件最大 1 MB');const rows=parseCSV(await file.text());const data=await api('/api/admin/import','POST',{rows,confirm:false});importRows=rows;const list=document.createElement('ol');for(const item of data.preview){const li=document.createElement('li');li.textContent=item.title+' · '+item.url;list.append(li);}const p=document.createElement('p');p.textContent=`共 ${data.preview.length} 条，确认后导入并发布。`;$('#import-preview').replaceChildren(p,list);$('#confirm-import').hidden=false;}catch(err){toast(err.message,true);}
});
$('#confirm-import')?.addEventListener('click',event=>busy(event.target,async()=>{const result=await api('/api/admin/import','POST',{rows:importRows,confirm:true});toast(`已导入 ${result.count} 条资源`);event.target.hidden=true;importRows=[];$('#csv-file').value='';$('#import-preview').textContent='导入完成，资源已发布。';}));
...
async function loadNetwork(){
 ...
 const list=textNode('div','','resource-list');for(const [index,item] of data.items.entries()){
 ...
 let tracked=false;
 const recordClick=event=>{if(tracked||!data.clickToken||(event.type==='auxclick'&&event.button!==1))return;tracked=true;fetch('/api/network-click',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({token:data.clickToken,index}),keepalive:true}).catch(()=>{});};
 row.addEventListener('click',recordClick);row.addEventListener('auxclick',recordClick);
